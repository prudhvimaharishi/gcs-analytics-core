...
class AnalyticsCacheManagerTest {
  ...
  void invalidateAll_withSchemaAccessHistory_clearsHistory() {
    ...
    manager.getSchemaAccessHistory().recordDataAccess(schemaFingerprint, "id");

    manager.invalidateAll();

    assertThat(manager.getSchemaAccessHistory().getDataColumns(schemaFingerprint)).isEmpty();
  }
}
