...
| `analytics-core.auth.proxy.address` | HTTP proxy address of the form `[https?://]hostname:port`. **Note**: Does not apply to the gRPC/bidi data plane, which relies on the standard JVM proxy environment. | - |
...
| `analytics-core.auth.http.connect-timeout-ms` | HTTP connect timeout for GCS HTTP-JSON data plane requests, in milliseconds. Must be positive. | `5000` |
| `analytics-core.auth.http.read-timeout-ms` | Socket read timeout for authentication and HTTP-JSON data plane HTTP requests, in milliseconds. Must be positive. | `5000` |
