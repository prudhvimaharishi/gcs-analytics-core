...
**When it turns off.** The Stream Optimizer runs only on `.parquet` files and needs the footer from the Footer Cache. If the file name does not end with `.parquet`, the footer cache is disabled, a small-file cache intercepts the footer read first, the footer is larger than the cached tail, or the footer is encrypted, the stream does not prefetch for that file.
...
flowchart LR
    subgraph Cold["❄️ COLD: first file, still learning columns"]
        ...
        subgraph CS["Single-Row-Group Files"]
            direction TB
            cs1["<b>① Footer Read</b><br/>Nothing: no columns<br/>learned yet"] --> cs2["<b>② Dictionary Read</b><br/>• Regular fetch<br/>• Add column to<br/>Dictionary Columns"] --> cs3["<b>③ Data Page Read</b><br/>• Add to Data Columns<br/>• Regular fetch of the RG"]
        end
        subgraph CM["Small-Row-Group Files"]
            direction TB
            cm1["<b>① Footer Read</b><br/>Nothing: no columns<br/>learned yet"] --> cm2["<b>② Dictionary Read</b><br/>• Regular fetch on first RG;<br/>cache hit on later RGs<br/>• Add to Dictionary Columns<br/>• Prefetch dict pages of later RGs<br/>• No data prefetch: no<br/>Data Columns known yet"] --> cm3["<b>③ Data Page Read</b><br/>• Add to Data Columns<br/>• Regular fetch on first RG;<br/>cache hit on later RGs<br/>• Prefetch next RG,<br/>using columns learned so far"]
        end
        subgraph CL["Large-Row-Group Files"]
            direction TB
            cl1["<b>① Footer Read</b><br/>Nothing: no columns<br/>learned yet"] --> cl2["<b>② Dictionary Read</b><br/>• Regular fetch on first RG;<br/>cache hit on later RGs<br/>• Add to Dictionary Columns<br/>• Prefetch dict pages of later RGs<br/>• No data prefetch: no<br/>Data Columns known yet"] --> cl3["<b>③ Data Page Read</b><br/>• Add to Data Columns<br/>• Regular fetch of RG k<br/>• No next-RG prefetch"]
        end
    end
    ...
    classDef gate fill:#ffe082,stroke:#ff8f00,color:#4e342e
    class cs1,cm1,cl1,wl1b,wm2b,wl2b,lg1 none
    class ws1,wm1a,wm1b,wl1a,lg2 pref
    class ws2,ws3,wl3,lg3 hit
    class cs2,cs3,cl3,wl4,lg4 fetch
    class wm2,wm3,wm3b,wl2,lg5 hitpref
    class cm2,cl2,cm3 fetchpref
...
| :--- | :--- | :--- |
| **Skipped in the dictionary sweep** | Some readers (such as Iceberg) check min/max for all row groups when the file opens, then read the dictionary pages of every row group that passed, before reading any data pages. | If the stream has seen dictionary page reads for row groups 0, 2, and 3 but zero for 1 (and all filter columns have dictionary pages in 1), then row group 1 failed the min/max filter. The stream skips it and prefetches row group 2. |
| **Min/max inside a rejected range** | The engine skipped an earlier row group, for example one where `price` ranges from 10 to 50. | If a later row group's range for any learned Dictionary Column (for example, `price` 20 to 40) sits entirely inside the skipped range, the stream predicts it will be skipped too. |
...
| **Bounded chunks and instant eviction** | Neighboring columns are merged into chunks of a capped size. Cached chunks served to batch (vectored) reads are dropped from the Prefetch Cache once the read reaches the end of the chunk. | Large heap spikes and garbage-collection pauses. |
...
| **Cancel unsubmitted queue entries** | When a file closes or a row group is skipped, prefetches still waiting in the deferred queue (not yet submitted to the thread pool) are dropped and their cache entries are evicted. | Wasting thread pool slots on stale prefetches that have not started yet. |
...
classDiagram
    ...
    GcsFooterOptimizer --> AnalyticsCacheManager : loads & caches footer
    PredictivePrefetchOptimizer --> AnalyticsCacheManager : gets footer, cache & history
    PrefetchScheduler --> PrefetchBufferCache : registers & evicts ranges
    PrefetchScheduler ..> PrioritizedReadExecutorService : low-priority reads (via GcsReadChannel)
```
...
| `read` / `afterRead` | Footer, dictionary, or data read (single buffer) | Loads the layout on first use. `read` serves from `PrefetchBufferCache.copyInto` and calls `observeAccess` on a cache hit; on a miss (or after `GcsFooterOptimizer` serves the footer), `afterRead` calls `observeAccess` once the foreground read completes. Footer reads go to `prefetchFirstRowGroupOnce`, dictionary page reads go to `onDictionaryPageRead` (dictionary de-duplication and the dictionary trigger), and data page reads record `SURVIVED` in `SchemaAccessHistory`, cancel skipped row groups, and call `speculateRowGroups`. |
| `readVectored` | Data read (vectored) | Serves each range from `PrefetchBufferCache.consumeRange`, then `recordVectoredAccess` records columns (and `SURVIVED` on the first data read), cancels skipped row groups, and picks the next surviving row group. |
| `afterReadVectored` | After the foreground vectored read is submitted | When no next row group is pending (before any data read, or after the last surviving row group), calls `prefetchFirstRowGroupOnce` (a no-op once the first row group was handled). Otherwise, unless the file is a large-row-group file, waits on the foreground range futures, then calls `speculateRowGroup` for the next surviving row group. |
...
When the engine reads data in row group `j`, `onDataPageRead` calls `PrefetchScheduler.cancelRangeWindow` on any unread row groups before `j` (from `0` on the first data read, or after the previous data read), because the engine skipped them. Dictionary reads never cancel prefetches.
...
| [`PredictivePrefetchOptimizer`](file:///usr/local/google/home/pbeerelly/Downloads/gcs-analytics-core/core/src/main/java/com/google/cloud/gcs/analyticscore/core/prefetch/PredictivePrefetchOptimizer.java) | `core`, per stream | Serves cached ranges, classifies reads, and decides what to prefetch. | Applicable only to `.parquet` objects. Large-row-group file when more than 1 row group and row group 0 is at least `64 MiB`; such files never pick a next row group (`shouldSpeculateNextRowGroup`). The dictionary trigger (`FIRST_DICT_READ` or `LAST_DICT_READ`) fires once per row group (`rowGroupsWithDataPrefetch`), only when no earlier row group's prefetch is still waiting for its data read (`hasPendingDataPrefetchBefore`), and marks the row group only when every data range is registered in the cache. Later dictionary pages are prefetched again only when `prefetchedDictionaryColumns` does not cover the known set; that set is recorded only when every range is registered in the cache and is cleared if any in-flight dictionary download fails (`forgetPrefetchedDictionariesOnFailure`). For known Dictionary Columns, dictionary ranges `[dictOffset, dataOffset)` are kept apart from data ranges `[dataOffset, endOffset)` while the dictionary page is registered in the cache; all other Data Columns use the full chunk `[startOffset, endOffset)` so full-chunk reads hit one contiguous entry. Merged chunks are capped at `block.size-bytes`. |
...
| [`PrefetchBufferCache`](file:///usr/local/google/home/pbeerelly/Downloads/gcs-analytics-core/client/src/main/java/com/google/cloud/gcs/analyticscore/client/PrefetchBufferCache.java), [`CachedRange`](file:///usr/local/google/home/pbeerelly/Downloads/gcs-analytics-core/client/src/main/java/com/google/cloud/gcs/analyticscore/client/CachedRange.java) | `client`, per filesystem instance | Byte-weighted Caffeine cache keyed by `(GcsItemId, startOffset)`, with a per-file offset index to find and stitch covering ranges. | `2 GiB` cap, `60s` idle expiry. Segments consumed by `consumeRange` are evicted once the read reaches `segment.endOffset`; `copyInto` does not evict (see Known Gaps). Exact matches on writable heap buffers return a zero-copy `duplicate()`. |
| [`PrioritizedReadExecutorService`](file:///usr/local/google/home/pbeerelly/Downloads/gcs-analytics-core/client/src/main/java/com/google/cloud/gcs/analyticscore/client/PrioritizedReadExecutorService.java), [`GcsReadChannel`](file:///usr/local/google/home/pbeerelly/Downloads/gcs-analytics-core/client/src/main/java/com/google/cloud/gcs/analyticscore/client/GcsReadChannel.java) | `client`, per filesystem instance (pool) / per stream (channel) | 16-thread priority pool and HTTP channel with promotion, exact ranges (`effectiveMaxMergeGap = 0`), and deferred-queue cancellation. | Low-priority tasks use at most `10` of `16` threads. Cancel removes unsubmitted tasks from `deferredLowPriorityQueue`; tasks already submitted to the pool finish normally. |
...
* Pool-thread callbacks: (1) `afterReadVectored` schedules the next row group once the foreground futures complete; `speculateRowGroup` is `synchronized` and checks the `volatile closed` flag, and `rowGroupsWithDataPrefetch` is a concurrent set. (2) `forgetPrefetchedDictionariesOnFailure` clears the `volatile prefetchedDictionaryColumns` if an in-flight dictionary download fails. (3) `tryCompleteFromCache` completes the caller's range, records hit/miss metrics, or falls back to the source channel when an in-flight range settles. (4) `PrefetchScheduler.schedule` completion callbacks record `PREFETCH_BYTES_LOADED` and remove finished offsets from the concurrent `inFlightRequests` map.
...
stateDiagram-v2
    ...
    Consumed: Consumed
    Consumed: Evicted once segment end is reached
    Expired: Expired
    Cancelled: Cancelled
    Cancelled: If in deferred queue, removed before pool submission
    Cancelled: Evicted from PrefetchBufferCache
    Failed: Failed
    Failed: Reader reads the bytes from GCS

    [*] --> InFlight: PrefetchScheduler.schedule()
    InFlight --> Ready: Download completes (or copyInto() waits on it)
    InFlight --> Consumed: consumeRange() waits on it
    Ready --> Consumed: consumeRange()
    Ready --> Ready: copyInto() keeps the range
    Ready --> Expired: 60s idle or 2 GiB cap
    InFlight --> Cancelled: Stream closed or row group skipped
    Ready --> Cancelled: Stream closed or row group skipped
...
| The wait for the foreground read covers only the vectored path. | `afterRead` calls `observeAccess` -> `speculateRowGroups` after the first single-buffer read of a row group. | Because non-vectored readers read a row group across multiple single-buffer calls, the next row group can start downloading while the remaining reads of the current row group are still running. |
| Unread Dictionary Columns keep the last-read trigger from firing. | `hasReadAllDictionaries` needs every known dictionary column, without checking if the column has a dictionary page in that row group, and `SchemaAccessHistory` never removes columns. | If a filter column falls back to plain encoding (no dictionary page) in a later row group, if min/max alone satisfies one column of an `AND` filter, or if the filesystem instance is reused across scans with different filters, the unread column keeps `LAST_DICT_READ` from firing. `FIRST_DICT_READ` avoids this. |
| Single-buffer cache hits do not evict. | `PrefetchBufferCache.copyInto` never removes ranges; only `consumeRange` does. | Bytes served to single-buffer reads stay until a later data read skips the row group, the stream closes, 60s idle, or the 2 GiB cap. For Iceberg these are mostly small dictionary pages, because its data reads are vectored. Evicting on read is not safe as is: the column chunk read re-reads the dictionary page first, and without that entry the whole chunk, including prefetched data pages, is fetched from GCS. |
...
| **Highly Selective Scans (90%+ File/Row-Group Pruning)** | **Zero scan regression and `XX%` reduction in shuffle wait overhead** by gating footer-read and dictionary-page-read speculation via the 8-file sliding window and dropping unsubmitted prefetches from the deferred queue. |

## Configuration, Telemetry, and Rollback

### Configuration Properties ([`GcsPrefetchOptions`](file:///usr/local/google/home/pbeerelly/Downloads/gcs-analytics-core/client/src/main/java/com/google/cloud/gcs/analyticscore/client/GcsPrefetchOptions.java), [`GcsCacheOptions`](file:///usr/local/google/home/pbeerelly/Downloads/gcs-analytics-core/client/src/main/java/com/google/cloud/gcs/analyticscore/client/GcsCacheOptions.java))
...
| `analytics-core.footer.cache.enabled` | `false` | Defined in `GcsCacheOptions`. Must be `true` (along with `analytics-core.footer.prefetch.enabled=true`, its default in `GcsReadOptions`) when `PREDICTIVE_ROW_GROUP` is enabled so `GcsFooterOptimizer` populates `AnalyticsCacheManager.getFooter(itemId)`. |
...
| :--- | :--- | :---: | :--- |
| `PREFETCH_CACHE_HIT` | `gcs.analytics-core.client.prefetch.cache.hits` | `COUNTER` | Foreground `read` calls or `readVectored` ranges served from `PrefetchBufferCache`. |
| `PREFETCH_CACHE_MISS` | `gcs.analytics-core.client.prefetch.cache.misses` | `COUNTER` | Foreground `read` calls or `readVectored` ranges that missed `PrefetchBufferCache` once the footer layout is loaded. |
...
| 2026-09-25 | [Prudhvi Beerelly](mailto:pbeerelly@google.com) | Aligned HLD Cold-file flowchart, Guardrail 2 signal rules, LLD class diagram, hook mappings, component contracts, pool-thread callbacks, range lifecycle state diagram, known gaps, and configuration/telemetry tables with the code. |
