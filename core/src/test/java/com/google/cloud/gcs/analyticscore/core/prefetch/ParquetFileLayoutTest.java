...
public class ParquetFileLayoutTest {
  ...
  void getRowGroup_knownOrdinal_returnsThatRowGroup() {
    ParquetRowGroup second = rowGroup(1, columnChunk("price", 1, 200, 50));
    ParquetFileLayout fileLayout = layout(rowGroup(0, columnChunk("price", 0, 100, 50)), second);
  ...
  void getRowGroup_negativeOrdinal_returnsEmpty() {
    ParquetFileLayout fileLayout = layout(rowGroup(0, columnChunk("price", 0, 100, 50)));
  ...
  @Test
  void getColumnPaths_returnsDistinctPathsAcrossRowGroups() {
    ParquetFileLayout fileLayout =
        layout(
            rowGroup(0, columnChunk("price", 0, 100, 50), columnChunk("name", 0, 150, 50)),
            rowGroup(1, columnChunk("price", 1, 200, 50)));

    assertThat(fileLayout.getColumnPaths()).containsExactly("price", "name");
  }

  @Test
  void equals_layoutsWithEqualRowGroups_areEqual() {
    ParquetFileLayout first = layout(rowGroup(0, columnChunk("price", 0, 100, 50)));
    ParquetFileLayout second = layout(rowGroup(0, columnChunk("price", 0, 100, 50)));
