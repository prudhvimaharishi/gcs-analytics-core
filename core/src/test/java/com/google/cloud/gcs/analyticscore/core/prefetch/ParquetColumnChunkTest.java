...
public class ParquetColumnChunkTest {
  ...
  void getEndOffset_returnsStartOffsetPlusCompressedSize() {
    ParquetColumnChunk chunk = columnChunk("price", 0, 100, 50);
  ...
  void hasDictionaryPage_chunkWithDictionary_returnsTrue() {
    ParquetColumnChunk chunk = dictionaryEncodedColumnChunk("price", 0, 100, 50, 120);
  ...
  @Test
  void contains_offsetAtStart_returnsTrue() {
    ParquetColumnChunk chunk = columnChunk("price", 0, 100, 50);

    boolean contains = chunk.contains(100);

    assertThat(contains).isTrue();
  }

  @Test
  void contains_offsetBeforeStart_returnsFalse() {
    ParquetColumnChunk chunk = columnChunk("price", 0, 100, 50);

    boolean contains = chunk.contains(99);

    assertThat(contains).isFalse();
  }

  @Test
  void contains_offsetAtEnd_returnsFalse() {
    ParquetColumnChunk chunk = columnChunk("price", 0, 100, 50);

    boolean contains = chunk.contains(150);
  ...
  void getDictionaryPageRange_dataPagesStartAtDictionaryPage_returnsEmpty() {
    ParquetColumnChunk chunk = dictionaryEncodedColumnChunk("price", 0, 100, 50, 100);

    assertThat(chunk.getDictionaryPageRange()).isEmpty();
  }

  @Test
  void getDataPageRange_spansDataPageOffsetToEndOffset() {
    ParquetColumnChunk chunk = dictionaryEncodedColumnChunk("price", 0, 100, 50, 120);

    Range<Long> dataPageRange = chunk.getDataPageRange();

    assertThat(dataPageRange).isEqualTo(Range.closedOpen(120L, 150L));
  }
}
