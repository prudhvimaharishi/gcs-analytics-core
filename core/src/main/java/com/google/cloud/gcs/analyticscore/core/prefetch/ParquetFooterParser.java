...
final class ParquetFooterParser {
   ...
   * @param fileSize the total size of the object in bytes
   */
  static Optional<ParquetFileLayout> parse(ByteBuffer tail, long fileSize) {
    ...
    try {
      ...
      return Optional.of(toFileLayout(fileMetaData, fileSize - TRAILER_LENGTH - metadataLength));
    } catch (Exception e) {
      return Optional.empty();
    }
  }
  ...
  private static ParquetFileLayout toFileLayout(FileMetaData fileMetaData, long footerStartOffset) {
    ...
    return ParquetFileLayout.create(
        computeSchemaFingerprint(fileMetaData), parsedRowGroups.build(), footerStartOffset);
  }

  private static ParquetRowGroup toRowGroup(RowGroup rowGroup, int ordinal) {
    ParquetRowGroup.Builder parsedRowGroup =
        ParquetRowGroup.builder().setOrdinal(ordinal).setRowCount(rowGroup.getNum_rows());
    for (ColumnChunk columnChunk : rowGroup.getColumns()) {
      toColumnChunk(columnChunk, ordinal).ifPresent(parsedRowGroup::addColumnChunk);
    }
    return parsedRowGroup.build();
  }

  private static Optional<ParquetColumnChunk> toColumnChunk(
      ColumnChunk columnChunk, int rowGroupOrdinal) {
    ...
    boolean hasDictionaryPage = dictionaryPageOffset > 0 && dictionaryPageOffset < dataPageOffset;

    ParquetColumnChunk.Builder parsedChunk =
        ParquetColumnChunk.builder()
            .setColumnPath(String.join(".", metaData.getPath_in_schema()))
            .setRowGroupOrdinal(rowGroupOrdinal)
            .setStartOffset(hasDictionaryPage ? dictionaryPageOffset : dataPageOffset)
