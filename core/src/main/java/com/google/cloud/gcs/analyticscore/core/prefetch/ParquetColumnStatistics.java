...
final class ParquetColumnStatistics {
   ...
   * {@code other}'s {@code [min, max]} interval.
   */
  boolean isContainedIn(ParquetColumnStatistics other) {
    ...
    return compareValues(this.minBytes, other.minBytes) >= 0
        && compareValues(this.maxBytes, other.maxBytes) <= 0;
  }

  private int compareValues(byte[] left, byte[] right) {
    switch (type) {
      case INT32:
        if (left.length == Integer.BYTES && right.length == Integer.BYTES) {
          return Integer.compare(toInt(left), toInt(right));
        }
