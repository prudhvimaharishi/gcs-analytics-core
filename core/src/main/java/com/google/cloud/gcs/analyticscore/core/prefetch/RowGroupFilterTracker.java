 ...
 * </ol>
 *
 * <p>Both signals can be wrong, for example when an earlier row group was rejected by a filter
...
final class RowGroupFilterTracker {
  ...
  OptionalInt findNextSurvivingRowGroup(
    ...
    for (int candidate = afterOrdinal + 1; candidate < rowGroupCount; candidate++) {
      if (isSkippedByUpfrontDictionarySweep(
          layout, candidate, afterOrdinal, maxDictOrdinal, activeFilterColumns)) {
        continue;
      }
  ...
  private boolean isSkippedByUpfrontDictionarySweep(
    ...
    if (maxDictOrdinal <= afterOrdinal || candidate > maxDictOrdinal) {
      return false;
    }
    Set<String> expectedColumns =
        dictionaryColumnsByRowGroup.getOrDefault(afterOrdinal, Collections.emptySet());
  ...
  private boolean allFilterColumnsHaveDictionaryPage(
      ParquetFileLayout layout, int ordinal, Set<String> activeFilterColumns) {
  ...
  private void markSkippedRowGroupsBefore(
    ...
    for (int ordinal = 0; ordinal < currentDataOrdinal; ordinal++) {
      if (dataTouchedOrdinals.contains(ordinal)
          || dictionaryTouchedOrdinals.contains(ordinal)
          || !processedSkippedOrdinals.add(ordinal)) {
        continue;
      }
