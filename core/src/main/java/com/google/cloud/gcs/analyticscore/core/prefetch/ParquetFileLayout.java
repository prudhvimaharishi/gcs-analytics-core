...
abstract class ParquetFileLayout {
  ...
  /** Returns the offset at which the footer metadata begins. */
  abstract long getFooterStartOffset();

  /** Creates a layout from the row groups described by the file footer. */
  static ParquetFileLayout create(
      int schemaFingerprint, ImmutableList<ParquetRowGroup> rowGroups, long footerStartOffset) {
    return new AutoValue_ParquetFileLayout(schemaFingerprint, rowGroups, footerStartOffset);
  }
  ...
  /** Returns every dotted column path present in this file. */
  final ImmutableSet<String> getColumnPaths() {
    return getRowGroups().stream()
        .flatMap(rowGroup -> rowGroup.getColumnChunks().keySet().stream())
