...
import com.google.cloud.gcs.analyticscore.client.CachedRange;
...
public final class PredictivePrefetchOptimizer implements FormatOptimizer {
  ...
  private boolean tryCompleteFromCache(GcsObjectRange range, IntFunction<ByteBuffer> allocate) {
    ...
    Optional<CachedRange> cachedRange =
        bufferCache.getRangeCovering(itemId, range.getOffset(), range.getLength());
    ...
    CompletableFuture<ByteBuffer> unused =
        cachedRange
            .get()
            .slice(range.getOffset(), range.getLength())
            .whenComplete(
                (sliceBuffer, error) -> {
                  if (error == null && sliceBuffer != null) {
                    ByteBuffer target = allocate.apply(range.getLength());
                    if (target != null) {
                      target.put(sliceBuffer);
                      target.flip();
                      range.getByteBufferFuture().complete(target);
                      telemetry.recordMetric(Metric.PREFETCH_CACHE_HIT, 1L, Collections.emptyMap());
                      telemetry.recordMetric(
                          Metric.PREFETCH_BYTES_CONSUMED,
                          range.getLength(),
  ...
  private void speculateRowGroups(
    ...
    List<Range<Long>> learnedRanges =
        collectLearnedRangesForRowGroups(rowGroupOrdinal, nextSurvivingOrdinal);
    ...
    scheduler.schedule(source, itemId, rangesAhead, fileSize);
  }
  ...
  private void speculateRowGroup(VectoredSeekableByteChannel source, int rowGroupOrdinal) {
    ...
    scheduler.schedule(source, itemId, ranges, fileSize);
  }
  ...
  private List<Range<Long>> collectLearnedRangesForRowGroups(
    ...
    collectRowGroupRanges(ranges, currentRowGroupOrdinal, dataColumns, dictionaryColumns);
    if (nextRowGroupOrdinal.isPresent()) {
      collectRowGroupRanges(ranges, nextRowGroupOrdinal.getAsInt(), dataColumns, dictionaryColumns);
    }
